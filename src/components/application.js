    ...
    <div className="fixed inset-0 bg-black bg-opacity-25 z-50 flex justify-center items-center">
      <div className="w-8/12 bg-white   shadow-xl h-3/4 overflow-y-scroll px-8 py-10">
        <div className="flex justify-between">
          <h1 className="text-2xl font-medium">Application</h1>
          <svg
            onClick={() => {
              setShowModal(false);
            }}
        ...
        <h1 className="text-xl font-medium text-green-600 my-6">
          Cover letter
        </h1>
        <p dangerouslySetInnerHTML={{ __html: item.coverLetter }}></p>

        <h1 className="text-xl font-medium text-green-600 mb-6">
        ...
        <h1 className="text-xl font-medium text-green-600 my-6">Skills</h1>
        <div className="flex flex-wrap">
          {item.skills.map((item) => {
            return (
              <div className="py-3 px-4 rounded-full bg-orange-50 font-medium me-2 mb-2 text-sm">
                {item}
              </div>
            );
          })}
        ...
        <h1 className="text-xl font-medium text-green-600 my-6">Experience</h1>
        <div className="space-y-2">
          {item.experiences.map((item) => {
            return <ExperienceItem item={item} />;
          })}
        </div>
        <h1 className="text-xl font-medium text-green-600 my-6">Referees</h1>
        <div className="space-y-2">
        ...
        <h1 className="text-xl font-medium text-green-600 my-6">CV/Resume</h1>
        <a
          href={item.cv}
          target="__blank"
        ...
        >
          ...
          <div className="font-medium mt-3">{item.fileName}</div>
        </a>
      </div>
    </div>
  );
