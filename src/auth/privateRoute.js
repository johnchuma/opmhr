...
const PrivateRoute = ({children}) => {
    ...
    return ( loading?<div className="flex   ms-auto w-full  justify-center items-center min-h-screen">
    <div className="border-4 rounded-full border-slate-900 h-8 w-8 animate-spin border-t-transparent "></div>
</div>: <div>
        {user == null?<Navigate to="/login" replace/>:<UserContext.Provider value={{ user }}>{children}</UserContext.Provider>}
    </div> );
}
